...
function calcAndShow() {
    ...
    if (isNaN(startSumm) || startSumm <= 0) {
        console.warn('no start sum value', startSumm)
        return
    }
    ...
    console.log(`invest(${startSumm}, ${plt}, ${action}) = `, [finishFor, addKesh, upKesh])
    $('#itogSumm').text( moneyFormat(finishFor) )

    let right = Math.max( upKesh / finishFor, 0.2 )
    let left = 1 - right
    $('#chart-left').css('width', (left*100)+'%');
    $('#chart-right').css('width', (right*100)+'%');

    $('#chart-left').text(moneyFormat( addKesh + startSumm ))
    $('#chart-right').text(moneyFormat( upKesh ))
...
$('#control-btns').click( e => {
    const btn = $(e.target);
    const text = btn.text();
    $('#control-btns .active').removeClass('active');
    btn.addClass('active');
    $('#onSumm').val(text);
    calcAndShow();
} )
